...
def _append_openai_port_preflight_lines(lines: list[str], *, cmd: str, expected_model: str) -> None:
    ...
    lines.append("import json, socket, sys, urllib.request")
    lines.append("port_str = sys.argv[1]")
    lines.append("expected = (sys.argv[2] or '').strip()")
    lines.append("try:")
    lines.append("    port = int(port_str)")
    lines.append("except ValueError:")
    lines.append("    sys.exit(0)")
    lines.append("is_occupied = False")
    lines.append("try:")
    lines.append("    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:")
    lines.append("        s.settimeout(0.5)")
    lines.append("        is_occupied = (s.connect_ex(('127.0.0.1', port)) == 0)")
    lines.append("except Exception:")
    lines.append("    pass")
    lines.append("if not is_occupied:")
    lines.append("    sys.exit(0)")
    ...
    lines.append("if models:")
    lines.append("    print(f'ERROR: Port {port} is already serving {models}.')")
    ...
    lines.append("else:")
    lines.append("    print(f'ERROR: Port {port} is already in use by another application (non-OpenAI service). Choose another port or stop the conflicting service.')")
    lines.append("sys.exit(98)")
    lines.append("PY")
